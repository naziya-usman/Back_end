...
async function main() {
    ...
    app.put("/employees/:id", (req, res) => {
        ...
        Employee.findByIdAndUpdate(employeedIdInRequest, update, {}).then((data) => {
            res.json(data);
        });
    });
    ...
    app.delete("/employees/:id", (req, res) => {
        ...
        Employee.findByIdAndDelete(employeedIdInRequest).then((result) => {
            res.json("Document deleted: succussfuly deleted ");
        });
    });
}

function validation(emplyName) {
    if (!emplyName.name || typeof emplyName.name != "string" || emplyName.name.trim() == "") {
